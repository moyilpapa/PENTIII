...
export default function EndpointsPage({ ctx }) {
  ...
  // Maps the classification scanner.py already computes per-path to a
  // color, so a soft-404/SPA-catch-all response no longer looks
  // identical to a genuinely reachable path in the table -- previously
  // this distinction only surfaced once you clicked into Path Detail.
  const classColor = (cls) => {
    switch (cls) {
      case "Reachable": return C.medium;          // worth a look
      case "Access Controlled": return C.accent;  // auth is doing its job, still note it
      case "Redirect": return C.fgDim;
      case "Soft 404 (catch-all response)": return C.fgDim;
      default: return C.fgDim;                    // Not Confirmed
    }
  };

  const softCount = rows.filter((r) => r.classification === "Soft 404 (catch-all response)").length;
          ...
          {result && (
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "3px 10px", background: C.panelB, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 10.5, color: C.fgDim, whiteSpace: "nowrap" }}>
                {result.checked}/{result.checked} paths · {result.found.length} reachable · {softCount} soft-404 · Complete
              </span>
            </div>
          )}
                ...
                <div style={{ flex: 1, overflowY: "auto" }}>
                  {rows.length === 0 ? (
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {rows.map((r, i) => {
                          const isSoft404 = r.classification === "Soft 404 (catch-all response)";
                          return (
                            <tr key={r.path} className={`trow${sel === i ? " selected" : ""}`} onClick={() => setSel(i)}>
                              <Td style={{ color: C.fgDim, textAlign: "center" }}>{i + 1}</Td>
                              <Td mono style={{ color: r.exists ? C.fg : C.fgDim, opacity: isSoft404 ? 0.55 : 1 }}>{r.path}</Td>
                              <Td mono style={{ color: statusColor(r.status_code), opacity: isSoft404 ? 0.55 : 1 }}>{r.status_code ?? "err"}</Td>
                              <Td mono style={{ color: classColor(r.classification), fontSize: 10.5 }}>{r.classification || "Not Confirmed"}</Td>
                              <Td mono style={{ color: C.fgDim, opacity: isSoft404 ? 0.55 : 1 }}>{r.content_length ?? "—"}</Td>
                            </tr>
                          );
                        })}
                      </tbody>
