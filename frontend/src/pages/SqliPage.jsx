...
function SqlmapPanel({ ctx }) {
  const { activeTarget, api, log, seedFinding } = ctx;
  const [url, setUrl] = useState(activeTarget.url);
  const [param, setParam] = useState("id");
  const [originalValue, setOriginalValue] = useState("1");
  const [running, setRunning] = useState(false);
  ...
  async function run() {
    ...
    setRunning(true);
    setResult(null);
    log("run", `real sqlmap → ${url} [${param}]`);
    try {
      const r = await api.scanSqlmap(activeTarget.id, { url, parameter: param.trim(), original_value: originalValue });
      setResult(r);
      if (!r.ok) log("fail", r.error);
      else log(r.vulnerable ? "fail" : "ok", r.vulnerable ? `sqlmap confirmed injection on "${param}"` : "sqlmap found no injection");
    } catch (e) {
      log("fail", e.message);
    } finally {
      setRunning(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      <Toolbar>
        ...
        <span style={{ fontSize: 11, color: C.fgDim, whiteSpace: "nowrap" }}>Param</span>
        <TInput value={param} onChange={setParam} style={{ width: 90, flex: "none" }} />
        <span style={{ fontSize: 11, color: C.fgDim, whiteSpace: "nowrap" }}>Value</span>
        <TInput value={originalValue} onChange={setOriginalValue} style={{ width: 70, flex: "none" }} />
        <TSep />
        <TBtn icon="play" label={running ? "Running sqlmap…" : "Run sqlmap"} accent disabled={running || !param.trim()} onClick={run} />
      </Toolbar>
      <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
        {!result && !running && (
          <EmptyHint>Run sqlmap against a parameter to see its verdict here.</EmptyHint>
        )}
        {result && !result.ok && (
          <div style={{ padding: 12, background: C.panelB, border: `1px solid ${C.border}`, borderRadius: 4, fontSize: 11.5, color: C.medium }}>
            {result.error}
          </div>
        )}
        {result?.ok && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: result.vulnerable ? C.critical : C.low }}>
                {result.vulnerable ? "SQL Injection Confirmed" : "No injection found"}
              </span>
              <span style={{ fontSize: 10.5, color: C.fgDim }}>{result.elapsed_seconds}s</span>
            </div>
            {result.vulnerable && (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <FieldRow label="Database" value={result.dbms || "unknown"} mono />
                <FieldRow label="Technique" value={result.type || "—"} />
                <FieldRow label="Title" value={result.title || "—"} />
                <FieldRow label="Payload" value={result.payload || "—"} mono />
                <TBtn
                  icon="add"
                  label="Add to Findings"
                  accent
                  onClick={() =>
                    seedFinding({
                      name: `SQL Injection confirmed by sqlmap — parameter "${param}"`,
                      location: url,
                      parameter: param,
                      severity: "High",
                      detection_method: `sqlmap (${result.type || "unknown technique"})`,
                      http_method: "GET",
                      confidence: "High",
                      description: result.title || "Confirmed by real sqlmap scan.",
                      evidence: `DBMS: ${result.dbms || "unknown"} | Payload: ${result.payload || "n/a"}`,
                    })
                  }
                />
              </div>
            ...
            <details style={{ marginTop: 14 }}>
              <summary style={{ fontSize: 10.5, color: C.fgDim, cursor: "pointer" }}>Raw sqlmap output</summary>
              <pre style={{ fontSize: 10.5, color: C.fgMid, whiteSpace: "pre-wrap", marginTop: 6, maxHeight: 240, overflowY: "auto" }}>
                {result.raw_output}
              </pre>
            </details>
          </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        {sub === "sqlmap" && <SqlmapPanel ctx={ctx} />}
      </div>
    </div>
  );
}
