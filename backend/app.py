...
ENABLE_SQLMAP = os.environ.get("ENABLE_SQLMAP", "").strip() == "1"
...
@app.route("/api/scan/sqli-sqlmap/<int:target_id>", methods=["POST"])
def scan_sqli_sqlmap(target_id):
    """Runs the real sqlmap tool (invoked as a subprocess, not embedded in
    this codebase -- see sqlmap_integration.py) against a specific
    URL/parameter. Entirely independent of sql_engine.py's own detection --
    both are available, neither depends on the other. If sqlmap isn't
    installed, returns a clear, non-fatal error explaining setup, rather
    than a 500."""
    target = get_target_or_404(target_id)
    if not target:
        return jsonify({"error": "not found"}), 404
    ...
    if not ENABLE_SQLMAP:
        return jsonify({
            "ok": False,
            "error": "sqlmap integration is disabled",
            "hint": "sqlmap execution has a larger blast radius than the rest of this API "
                    "(it's a full external tool with its own attack techniques), so it's off "
                    "by default. Restart the API with ENABLE_SQLMAP=1 to enable it.",
        }), 403
    ...
    mark_scan_started(target_id)
    result = sqlmap_integration.run_sqlmap(url, param, original_value, config)
    if result.get("ok"):
        save_scan_result(target_id, "sqli_sqlmap", result)
    return jsonify(result)
